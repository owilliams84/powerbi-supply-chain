...
Page 06 is late deliveries as a heat-mapped calendar: late orders by the day the order was
placed, with the orders shipped that day in the corner of each cell. Buttons zoom out from a
Monday-first month grid to the months of a year, the quarters of every year, and the years.
...
Every cell of all four views - 1,181 of them, value and shade band - is read out of the live
model and diffed against pandas with no DAX involved: 2,362 checks, no mismatches. The first
run had one: a year with a single month of data shaded as the lowest band, because
`DIVIDE(BLANK, 0, alt)` returns blank, not `alt`.
