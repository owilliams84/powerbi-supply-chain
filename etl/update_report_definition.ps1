...
$staging = Join-Path $env:TEMP "supplychain-report-update"
...
$body = Join-Path $env:TEMP "supplychain-update-body.json"
...
for ($i = 0; $i -lt 30; $i++) {
    ...
    $out = Join-Path $env:TEMP "supplychain-getdef.json"
    & az rest --method post --resource $FABRIC `
